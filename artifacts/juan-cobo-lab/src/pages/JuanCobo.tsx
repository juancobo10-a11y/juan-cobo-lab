  ...
  return (
    <div className="min-h-screen bg-[#F9F9F7]">
      ...
      <main id="main-content">
        ...
        {/* ── 1. HERO — dos columnas (texto izq · retrato der) ─── */}
        <section className="bg-[#0D1B2A] pt-24 pb-28 px-6 relative overflow-hidden">
          {/* Grid decorativo */}
          <div
          ...
          <div className="max-w-6xl mx-auto relative">
            <div className="flex flex-col md:flex-row items-center gap-14 md:gap-20">
              ...
              {/* Columna texto — orden 1 siempre */}
              <motion.div
                className="flex-1 text-center md:text-left order-1"
                initial="hidden"
                animate="visible"
              ...
              >
                ...
                <motion.h1
                  variants={fadeUp}
                  className="text-4xl md:text-5xl lg:text-6xl font-serif text-white leading-[1.05] mb-8"
                >
                  ¿Quién es Juan Cobo?
                </motion.h1>
                <motion.p
                  variants={fadeUp}
                  className="text-lg md:text-xl text-white/65 font-light leading-relaxed max-w-xl mx-auto md:mx-0 mb-6 italic font-serif"
                >
                ...
                >
                  Economista · Doctor en Ciencias Sociales · Investigador<br />
                  Servidor público · Profesor · Aprendiz permanente
                </motion.p>
              </motion.div>

              {/* Retrato editorial — orden 2 en móvil (debajo del texto), orden 2 en desktop (derecha) */}
              <motion.div
                className="flex-shrink-0 order-2 w-56 sm:w-64 md:w-72 lg:w-80"
                initial="hidden"
                animate="visible"
                variants={fadeIn}
              >
                <div className="relative">
                  ...
                  <div className="absolute -bottom-3 -right-3 w-full h-full rounded-2xl bg-accent/20 blur-sm" />
                  <img
                    src={`${import.meta.env.BASE_URL}images/juan-cobo.jpg`}
                    alt="Juan Carlos Cobo Gómez — fundador de Juan Cobo Lab"
                    className="relative w-full aspect-[3/4] object-cover object-top rounded-2xl shadow-2xl shadow-black/40"
                    loading="eager"
                    decoding="async"
                  />
                </div>
        ...
        {/* ── 3. BLOQUE MARTINA — texto izq · foto der ────────── */}
        <section className="bg-[#0D1B2A] py-28 px-6">
          <div className="max-w-5xl mx-auto">
            <motion.div
              className="flex flex-col md:flex-row items-start gap-14 md:gap-20"
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true, margin: '-80px' }}
              variants={{ visible: { transition: { staggerChildren: 0.15 } } }}
            >
              {/* Texto — orden 1 siempre */}
              ...
              {/* Foto Martina — orden 2 (debajo en móvil, derecha en desktop) */}
              <motion.div
                variants={fadeIn}
                className="order-2 flex-shrink-0 w-full max-w-[260px] md:w-56 lg:w-64 mx-auto md:mx-0"
              >
                <img
                  src={`${import.meta.env.BASE_URL}images/juan-martina.jpg`}
                  alt="Juan Cobo junto a Martina"
                  className="w-full aspect-[3/4] object-cover object-top rounded-2xl shadow-2xl shadow-black/50"
                  loading="lazy"
                  decoding="async"
                />
                <p className="mt-4 text-sm text-white/35 italic text-center leading-snug">
                  Martina. Mi mayor proyecto de vida.
                </p>
              </motion.div>
            </motion.div>
          </div>
                  ...
                  {cycleSteps.map((step, i) => (
                    <li key={step} className="flex items-center flex-1 min-w-0">
                      <div className="flex flex-col items-center flex-1 min-w-0">
                        <div
                          className="w-9 h-9 rounded-full bg-[#0D1B2A] border-2 border-accent flex items-center justify-center mb-3 flex-shrink-0"
                          aria-hidden="true"
                        >
                ...
                <ol aria-label="Ciclo de método" className="md:hidden flex flex-col gap-3 list-none">
                  {cycleSteps.map((step, i) => (
                    <li key={step} className="flex items-center gap-4">
                      <div
                        className="w-8 h-8 rounded-full bg-[#0D1B2A] border-2 border-accent flex items-center justify-center flex-shrink-0"
                        aria-hidden="true"
                      >
                        <span className="text-[10px] font-bold text-accent">{i + 1}</span>
                      </div>
