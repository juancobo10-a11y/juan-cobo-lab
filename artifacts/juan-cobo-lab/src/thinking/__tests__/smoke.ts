 ...
 *
 * v0.4.0 (S-010): adds ExplanationService tests (fuentePrincipal,
 * dimensionesDetectadas), NEUTRAL_TERMS filtering verification.
 *
 * Usage:
    ...
    return candidates
      .map((c) => {
        const terms = c.terminosCoincidentes
          .map(
            (t) =>
              `${t.esFrase ? "📌" : "·"} "${t.termino}" [${t.campo}, ${t.peso}, ${t.superficie}]`
          )
          .join("\n      ");
        return (
...
const CASES: {
  ...
  {
    // Sistémico con keywords claras en el texto del problema.
    // fuentePrincipal debe ser 'problema'; dims deben incluir términos sistémicos.
    router: prodRouter,
    input: {
      texto:
    ...
    label: "[S-010] Sistémico directo → fuentePrincipal='problema', dims sistémicas detectadas",
  },
  {
    // Socrático con keywords explícitas (supuestos + evidencia).
    // fuentePrincipal debe ser 'problema'; dims deben incluir supuestos y/o evidencia.
    router: prodRouter,
    input: {
      texto:
    ...
    label: "[S-010] Socrático directo → fuentePrincipal='problema', dims socrática detectadas",
  },
  {
    // Consulta genérica sin señales de ningún patrón → fallback universal.
    // fuentePrincipal debe ser 'fallback'; resumen debe declarar ausencia de señales.
    router: prodRouter,
    input: {
      texto: "¿Por qué no funciona bien la inversión pública en el sector?",
    },
    ...
    label: "[S-010] Fallback → fuentePrincipal='fallback', resumen declara ausencia de señales",
  },
  {
    // Texto con señal sistémica débil en el problema + pack con vocabulario
    // sistémico fuerte → fuentePrincipal 'mixta' (ambas superficies contribuyen).
    router: prodRouter,
    input: {
    ...
    expectedDimMinCount: 2,
    label: "[S-010] Mixta — problema + pack contribuyen → fuentePrincipal='mixta'",
  },
];
...
console.log("\n═══════════════════════════════════════════════════════════");
console.log("  HELIOS · Thinking Engine — Smoke Tests (v0.4.0 / S-010)");
console.log(
...
for (let i = 0; i < CASES.length; i++) {
  ...
  if (tc.expectedResumenContains) expectedStr += `  resumen⊃"${tc.expectedResumenContains}"`;
  ...
  if (explicacion) {
    console.log(`   Resumen  : "${explicacion.resumen.slice(0, 100)}${explicacion.resumen.length > 100 ? "…" : ""}"`);
  }
  console.log(`   Scores   :\n${scores}`);
  console.log();
}
...
// ─── Explicación no expone scores ni constantes internas ─────────────────────

console.log();
console.log("── Explicación: sin scores, pesos ni constantes internas ──");
{
  const r = await new ThinkingRouter(new KeywordThinkingAlgorithm()).route({
    texto:
  ...
  if (r.decision === "seleccionado") {
    ...
    const noExposeScore = !/\d+\.\d+/.test(resumen);
    const noExposeWeight = !/(peso|weight|SCORING|THRESHOLD|ninguna|baja|alta)/.test(resumen);
    const noExposeKeyword = !/(keyword|campo|esFrase|superficie)/.test(resumen);
    const allClean = noExposeScore && noExposeWeight && noExposeKeyword;
    if (allClean) passed++; else failed++;
    console.log(`${allClean ? "✅" : "❌"} Resumen no expone scores/pesos/constantes internas`);
    if (!noExposeScore) console.log("   ⚠ encontró número decimal");
    if (!noExposeWeight) console.log("   ⚠ encontró término interno de peso/umbral");
    if (!noExposeKeyword) console.log("   ⚠ encontró término de campo interno");
  } else {
    failed++;
    console.log("❌ No se obtuvo resultado seleccionado para verificar resumen");
  }
}
